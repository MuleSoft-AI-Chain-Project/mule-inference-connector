...
public class GeminiConverterHelper {
  ...
  public static GeminiParameters convertToGeminiParameters(com.mulesoft.connectors.inference.api.request.Parameters original) {
    Map<String, GeminiProperty> convertedProps = new HashMap<>();
    original.properties().forEach((k, v) -> {
      convertedProps.put(k, new GeminiProperty(v.type(), v.description(), v.enumValues()));
    });
